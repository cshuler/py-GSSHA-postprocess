...
def make_rain_gag_file(PrjName, Input_Precip_df, Precip_column_Name, StartDate, EndDate, Lat, Lon, 
                        RainSeries_timestep_Mins, PrjDir = os.path.join('.', 'RUN'), GageName="mooface", ImpPrecip_units="Inches"):
...
def Plot_and_Compare(OutHydro, SlicedStreamflow_df, Rain_Data_Frame, RunID, elapsed, 
    ...
    Rain_Data_Frame['rain_in_mm'] = Rain_Data_Frame['rain_in_mm'].apply(lambda x: float(x)) 
    fig, ax = plt.subplots(figsize=(8, 3))
    ax2 = ax.twinx()
    lns0 = ax2.plot(Rain_Data_Frame['rain_in_mm'], '.', c='b', alpha=0.4, label="Rainfall ")   # Plot rainfall 
